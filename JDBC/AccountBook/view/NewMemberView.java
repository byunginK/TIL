...
public class NewMemberView extends JFrame implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (btnTitle.equals("중복")) {
			// id 확인

			String id = tx1.getText().trim(); // trim 앞 뒤 여백을 없애주는것
			boolean b = dao.getId(id);

			if (b == true) {
				JOptionPane.showMessageDialog(null, "이미 있는 ID 입니다. 다른 ID를 입력하세요.");
				tx1.setText("");
			} else {
				JOptionPane.showMessageDialog(null, "이 ID는 사용 가능합니다.");
			}
		}
		if (btnTitle.equals("회원가입")) {
			if(tx1.getText().equals("") || tx2.getText().equals("") || tx3.getText().equals("") || tx4.getText().equals("")) {
				...
				String email = tx4.getText().trim();
	
				if (dao.getId(id)) {
					JOptionPane.showMessageDialog(null, "이미 있는 ID 입니다. 다른 ID를 입력하세요.");
				}
				else {
					int num = dao.addMember(id, pw, name, email);
					if (num == 1) {
